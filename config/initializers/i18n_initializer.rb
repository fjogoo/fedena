if File.exists?(File.expand_path("./config/locale_settings.yml"))
  ...
  AVAILABLE_LANGUAGES = (locale_settings['available'].length > 0) ? 
locale_settings['available'] : { :en => 'English' }
  DEFAULT_LANGUAGE = (AVAILABLE_LANGUAGES.include?(locale_settings['default'])) ? 
locale_settings['default'] : AVAILABLE_LANGUAGES.keys[0].to_s
